...
/// <summary>
/// Class that manages pages of UI elements and the game's UI
/// </summary>
public class UIManager : MonoBehaviour
{
    [Header("Page Management")]
    [Tooltip("The pages managed byt the UI Manager")]
    public List<UIPage> pages;
    [Tooltip("The index of the active page in the UI")]
    public int currentPage = 0;
    [Tooltip("The page (by index) switched to when the UIManager starts up")]
    public int mainMenu = 0;
    public int gamePage = 1;
    ...
    {
        if (GameManager.instance != null && GameManager.uiManager == null)
        {
    ...
    {
        if (GameManager.gameState == GameManager.GameState.MainMenu || GameManager.gameState == GameManager.GameState.GameOver || GameManager.gameState == GameManager.GameState.Pause)
        {
            GoToPage(gamePage);
            isPaused = false;
    ...
    {
        if (GameManager.gameState == GameManager.GameState.GameOver || GameManager.gameState == GameManager.GameState.Pause)
        {
            GoToPage(mainMenu);
        }
    }
